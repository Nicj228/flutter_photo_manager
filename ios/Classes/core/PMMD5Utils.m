...
+ (NSString *)getSHA256FromString:(NSString *)string {
    ...
    NSMutableString *outputStr = [NSMutableString stringWithCapacity:CC_SHA256_DIGEST_LENGTH * 2];
...
+ (NSString *)getSHA256FromPath:(NSString *)path {
    return (__bridge_transfer NSString *) PMSHA256HashFromPath((__bridge CFStringRef)path, PMFileHashDefaultChunkSizeForReadingData);
}

CFStringRef PMSHA256HashFromPath(CFStringRef filePath, size_t chunkSizeForReadingData) {
    ...
    while (hasMoreData) {
        ...
        CC_SHA256_Update(&hashObject, 1  (const void *)buffer, (CC_LONG)readBytesCount);
    }
